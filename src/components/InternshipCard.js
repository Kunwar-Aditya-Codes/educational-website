...
const InternshipCard = ({ title, p1, time, p2 }) => {
  return (
    <div className="border text-center rounded-md p-4 w-[95%] mx-auto my-2">
      ...
      <p className="mt-3 text-lg text-white">{p1}</p>
      <p className="mt-3 text-lg text-white">Taught by Industry Expert</p>
      <p className="my-1 text-lg text-white">Time : {time} months</p>
    </div>
  );
};
